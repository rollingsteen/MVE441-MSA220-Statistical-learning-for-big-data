...
n_sig <- c()
n_sig_test<-c()
alpha <- c(0.1,0.05,0.01,0.001,0.0001,0.00001,0.00000001)
test_error <-c()
...
for (ii in 1:5){
  ...
  for(fold in 1:5){
    ...
    for(kk in 1:length(alpha)){
      ...
      #  tuneGrid = data.frame(k = 3)  # You can tune k if desired
      #)
      ...
      #Fit lda
      #fit <- lda(labels.train~.,data=as.data.frame(data.train.dimred))#Change this function to the method you want to use 🙂
      
      pred <- predict(fit,newdata=as.data.frame(data.validation.dimred))
      errors[kk] <- errors[kk] + sum(pred!=labels.validation) 
    }
  }    
  training_errors[[ii]] <- errors
  ...
  n_sig_test[ii] <- length(sig_p.val)
  ...
  #Fit lda
  #fit <- lda(labels.trainvalidation~.,data=as.data.frame(data.trainvalidation.dimred))
  pred <- predict(fit,newdata=as.data.frame(data.test.dimred))
  test_error[ii] <- sum(pred!=labels.test)/length(labels.test)
  
}

boxplot(test_error,ylab="error rate",main="error rate for RF with F-test (n=1000)")
...
image(
  ...
  xlab = "", ylab = "", main="Selected pixels RF with F-test (n=200)", axes = FALSE, asp = 1
)
print(length(sig_p.val))
boxplot(n_sig_test, main='Number of selected features RF with F-test (n=1000)')

plot(log(alpha),training_errors[[1]]/640,type='l',ylim=c(0,0.5),ylab='Training Error',xlab='log(alpha)')
for(i in 2:5){
  lines(log(alpha),training_errors[[i]]/640)
}

n_sig_200 <- n_sig_test

boxplot(n_sig_200,n_sig_1000,names=c('n=200','n=1000'),main='Number of selected features')
