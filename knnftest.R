...
boxplot(test_error,ylab="error rate",main="error rate for KNN with")

raster_vec <- rep(1, 64 * 64)

# Set some random pixel indices to black (0)
raster_vec[sig_p.val] <- 0

# Convert the vector to a 64x64 matrix
img <- matrix(raster_vec, nrow = 64, ncol = 64, byrow = TRUE)

# Display the image with proper orientation
image(
  1:64, 1:64, t(apply(img, 2, rev)),  # Rotate for correct orientation
